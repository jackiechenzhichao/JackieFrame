...
namespace fs
{
    ...
    public sealed class TCPClientSocket
    {
        ...
        public void Close()
        {
            lock (m_sync_lock)
            {
                if (m_socket != null)
                {
                    ...
                    m_socket.Close();
                    m_socket = null;
                    Debuger.Log("TCPClientSocket 关闭");
                }
                if (OnClose != null)
                {
                    OnClose.Invoke(0);
        ...
        public bool Connect(string ip, ushort port)
        {
            ...
            try
            {
                Debuger.Log("TCPClientSocket - Connect");
        ...
        private void OnConnected(IAsyncResult ar)
        {
            ...
            try
            {
                lock (m_sync_lock)
                {
                    ...
                    this.BeginReceive();
                    Debuger.Log("TCPClientSocket - OnConnected");
                    if (OnOpen != null) OnOpen.Invoke(0);
                }
            }
            catch (Exception e)
            {
                Debuger.LogWarning("连接失败:" + e.Message);
                this.Close();
            }
        }
        ...
        public void Send(byte[] message, int offset, int count)
        {
            ...
            {
                Debuger.LogError("发送失败:" + e.Message);
                this.Close();
            }
        }
