...
namespace fs
{
    ...
    */
    ...
    public class UIGridWrapContent : MonoBehaviour
    {
        #region  排版相关
        ...
        public UIGridWrapContentConfig mConfig;
        public UIGridArrangeBase mGridArrangeBase;
        ...
        //事件监听
        public UIGridEventListenter mUGUIGWCEventListenter { get; private set; }
        ...
        void Update()
        {
            ...
            //Debuger.LogError(mScrollRect.velocity);
            ...
            {
                //if (mPosChange == false)
                //    Debuger.LogError("Positin 改变, 位置 = ");
            ...
            else if (mPosition == this.transform.position && mPosChange)
            {
                mPosChange = false;
                OnPosEndChange();
                // Debuger.LogError("Positin 停止改变, 位置 = ");
        ...
        private void RefreshAllCellPos()
        {
            ...
            while (true)
            {
                ...
                if (i > 150)
                {
                    Debuger.LogError("cell 刷新位置时，循环次数超出150次， 不合理， 已退出");
                    return;
                }
                ...
                for (; tOldI < mDataIndexList.Count; ++tOldI)
                {
                    ...
                    //老的隐藏回调
                    if (mConfig.mConcealCellAction != null)
                        mConfig.mConcealCellAction(tOldDataIndex, tCell);
        ...
        private void CacheData()
        {
            ...
            mUGUIGWCEventListenter = mScrollRect.GetComponent<UIGridEventListenter>();
            if (mUGUIGWCEventListenter == null)
                mUGUIGWCEventListenter = mScrollRect.gameObject.AddComponent<UIGridEventListenter>();
        ...
        private UIGridArrangeBase GetGridArrangeInstance()
        {
            switch (mArrangeType)
            {
                case ArrangeType.Horizontal:
                    return new UIGridArrangeHorizontal(this);

                case ArrangeType.HorizontalPage:
                    return new UIGridArrangeHorizontalPage(this);

                case ArrangeType.Vertical:
                    return new UIGridArrangeVertical(this);
            }

            Debuger.LogError("匹配不到对应的排序算法类  mArrangeType = " + mArrangeType);
            return null;
        }
        ...
        public void Show(UIGridWrapContentConfig pConfig)
        {
            mConfig = pConfig;
            SetDefaultValue();
            DelayCall(() =>
        ...
        private Vector2 GetCellOffsetValue(RectTransform pCell)
        {
            ...
            {
                Debuger.LogError("cell 的锚点必须是标准的形式， 就是TopLeft, TopCenter ... ... 这种");
            }
#endif
        ...
        private void TrySupplementDrag(Vector2 pDragPosOffset, float pDragTimeOffset)
        {
            ...
            //  Debuger.LogError(string.Format("index = {0}  tTargetPos = {1}", tDragSupplementIndex, tTargetPos));
        }
        ...
        private void UpdateScrollToTarget()
        {
            ...
            //Debuger.LogError(string.Format("CurPos = {0}  mScrollTargetPos = {1}  tLerpPos = {2}",
        ...
        public void RepositionCellInEditor()
        {
            ...
            {
                this.mConfig = new UIGridWrapContentConfig();
                mConfig.mDataCnt = 30;
            }
...
#if UNITY_EDITOR
    [CustomEditor(typeof(UIGridWrapContent))]
    public class UIGridWrapContentInspector : Editor
    {
        public override void OnInspectorGUI()
        {
            UIGridWrapContent tGridWrapContent = target as UIGridWrapContent;
            ...
            {
                tGridWrapContent.mArrangeType = (UIGridWrapContent.ArrangeType)EditorGUILayout.EnumPopup("排列方式(ArrangeType)", tGridWrapContent.mArrangeType);
            }
            GUI.enabled = true;

            if (tGridWrapContent.mArrangeType == UIGridWrapContent.ArrangeType.Horizontal)
            {
            ...
            {
                case UIGridWrapContent.ArrangeType.Vertical:
                ...
                case UIGridWrapContent.ArrangeType.Horizontal:
                ...
                case UIGridWrapContent.ArrangeType.HorizontalPage:
                    tGridWrapContent.mVerticalCnt = EditorGUILayout.IntField(new GUIContent("Vertical Count", "水平方向，每行有几个"), tGridWrapContent.mVerticalCnt);
                    tGridWrapContent.mHorizontalCnt = EditorGUILayout.IntField(new GUIContent("Horizontal Count", "垂直方向，每列有几个"), tGridWrapContent.mHorizontalCnt);
