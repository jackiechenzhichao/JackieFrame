...
namespace AntiCheat.Detectors
{
	...
	public class SpeedHackDetector: MonoBehaviour
	{
		...
		private void Awake()
		{
			...
			{
				Debuger.LogWarning("[ACT] " + COMPONENT_NAME + " is placed in scene incorrectly and will be auto-destroyed! Please, use \"GameObject->Create Other->Code Stage->Anti-Cheat Toolkit->" + COMPONENT_NAME + "\" menu to correct this!");
				Destroy(this);
				return;
			}
		...
		private void OnTimer()
		{
			...
			{
				errorsCount++;
				Debuger.LogWarning("[ACT] SpeedHackDetector: detection! Silent detections left: " + (maxFalsePositives - errorsCount));
